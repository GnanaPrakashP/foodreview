...
--
-- The historical root migrations in this repository were written as additive
-- changes against an existing FoodReview database. A clean local Supabase reset
-- needs the production core objects that those migrations already assumed:
-- profiles, reviews, circle relationships, review media rows, comments, likes,
-- visibility helpers, and review media buckets. This migration creates only
-- missing objects and does not drop, rename, or rewrite existing data.
