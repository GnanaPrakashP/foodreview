...
### Bucket `review-media-quarantine`

Private pending bucket for new Profile avatar and review/post image uploads.

Prefixes:

- `pending/{user_id}/...`: generated by the trusted upload-intent route.

Rules:

- Pending media is not publicly readable.
- Authenticated users may upload only to the exact owner/category/path authorized by a live upload intent.
- Finalization is performed by trusted server code, which downloads from quarantine, decodes with Sharp, strips metadata, normalizes orientation, re-encodes to JPEG, writes the canonical finalized object, and deletes the quarantine object.
...
- `public/{user_id}/...`: finalized, server-validated public media.

Mobile rule:

- Never upload pending media directly to `review-photos`.
- Never supply final public URLs or final storage paths from mobile.
- New review/post video uploads are disabled until trusted transcoding and metadata stripping exists.
