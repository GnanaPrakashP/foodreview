...
test("public pages render the app shell", async ({ page }) => {
  ...
  await expect(page).toHaveTitle(/Witoh/i);
  await expect(page.locator("body")).toContainText(/Witoh|circle|eating/i);
});

test("login page supports email sign-in flow", async ({ page }) => {
  await page.goto("/login");

  await expect(page.getByRole("heading", { name: /Witoh/i })).toBeVisible();
