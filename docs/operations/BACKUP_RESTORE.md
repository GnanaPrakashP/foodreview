...
`npm run backup:restore-drill:local` performs a real custom-format `pg_dump`, restores it into a randomly named temporary PostgreSQL database inside the running local Supabase database container, verifies migration head `202607160001`, runs canonical schema and operations missing-table/RLS checks, verifies the Phase 1A–5 critical domains, then drops the temporary database and dump. It never points at production and requires the explicit `PHASE7_LOCAL_RESTORE_DRILL` confirmation token.
