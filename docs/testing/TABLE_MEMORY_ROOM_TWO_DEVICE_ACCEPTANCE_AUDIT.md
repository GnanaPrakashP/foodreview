...
| Video upload from A and B | FAIL | Each uploader retained a noncanonical progress item and the peer never received it. `/private/tmp/tmr-fail-video-stuck-phone-a.png`, `/private/tmp/tmr-fail-video-missing-phone-b.png`, `/private/tmp/tmr-video-b-missing-phone-a.png`, `/private/tmp/tmr-video-b-stuck-phone-b.png` |
| Chat/Media consistency | PASS for images; FAIL for videos | Two canonical images matched across both tabs/phones; neither video completed canonical attachment |
...
| ID | Area | Preconditions | Phone A action | Phone B expected result | Phone A expected result | Realtime required | Restart verification | Offline verification | Result | Evidence | Defect |
| -- | ---- | ------------- | -------------- | ----------------------- | ----------------------- | ----------------- | -------------------- | -------------------- | ------ | -------- | ------ |
| J-01 | Media surfaces | Tabs observed | Send chat image/media | Chat and Media share canonical item; Overview media count updates | Same | Yes | Same after restart | Reconnect catches all surfaces | FAIL | Images were canonical and consistent on both; both videos failed to canonicalize/appear on peers | Hosted video-processing failure |
...
| P1 | Hosted video processing returned `moderation_service_unavailable` for uploads from both phones. Root cause is missing worker audit-identity hashing configuration after moderation approval. Readiness, error classification and stage timing are corrected locally, but the worker secret/configuration and build are not deployed. | Root cause fixed in source; hosted/two-phone retest BLOCKED |
