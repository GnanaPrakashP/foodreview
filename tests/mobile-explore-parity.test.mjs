...
test("mobile explore tabs use a native horizontal pager with virtualized panels", () => {
  const explore = source("mobile/app/(tabs)/explore.tsx");
  ...
  assert.match(explore, /from "react-native-reanimated"/);
  assert.match(explore, /const pagerRef = useAnimatedRef<ScrollView>\(\)/);
  assert.match(explore, /const scrollX = useSharedValue\(tabIndexFor\(initialTab\) \* width\)/);
  assert.match(explore, /const keepLoadingPanelsDuringTransitionRef = useRef\(false\)/);
  assert.match(explore, /const \[, forceRenderAfterPagerTransition\] = useState\(0\)/);
  assert.match(explore, /const renderLoadingPanels = !normalizedQuery && \(showLoading \|\| keepLoadingPanelsDuringTransitionRef\.current\)/);
  assert.match(explore, /renderLoadingPanels \? EMPTY_PLACES : buildPlaces\(posts\)/);
  assert.match(explore, /renderLoadingPanels \? EMPTY_DISHES : buildDishes\(posts\)/);
  assert.match(explore, /renderLoadingPanels \? EMPTY_PEOPLE : buildPeople\(posts, viewerName, viewerDisplayName\)/);
  assert.match(explore, /const pagerScrollHandler = useAnimatedScrollHandler\(\{/);
  assert.match(explore, /scrollX\.value = event\.contentOffset\.x/);
  assert.match(explore, /function ExploreLoadingPanel\(\{ tab \}: \{ tab: ExploreTab \}\)/);
  assert.match(explore, /if \(showLoading\) keepLoadingPanelsDuringTransitionRef\.current = true/);
  assert.match(explore, /withTiming\(nextOffset,\s*\{ duration: 260, easing: Easing\.out\(Easing\.cubic\) \}\s*\)/);
  assert.match(explore, /runOnUI\(\(x: number, shouldAnimate: boolean\) => \{/);
  assert.match(explore, /scrollTo\(pagerRef, x, 0, shouldAnimate\)/);
  assert.doesNotMatch(explore, /pagerRef\.current\?\.scrollTo/);
  assert.doesNotMatch(explore, /setActiveTab\(tab\);\s*scrollX\.value = withTiming/);
  assert.match(explore, /function ExploreTabs\(/);
  assert.match(explore, /scrollX: SharedValue<number>/);
  assert.match(explore, /const indicatorStyle = useAnimatedStyle\(\(\) => \{/);
  assert.match(explore, /Extrapolation\.CLAMP/);
  assert.match(explore, /<Reanimated\.ScrollView/);
  assert.match(explore, /horizontal/);
  assert.match(explore, /pagingEnabled/);
  assert.match(explore, /contentOffset=\{\{ x: tabIndexFor\(initialTab\) \* width, y: 0 \}\}/);
  assert.match(explore, /onScrollBeginDrag=\{handlePagerScrollBegin\}/);
  assert.doesNotMatch(explore, /onMomentumScrollBegin=\{handlePagerScrollBegin\}/);
  assert.match(explore, /onMomentumScrollEnd=\{handlePagerMomentumEnd\}/);
  assert.match(explore, /onScroll=\{pagerScrollHandler\}/);
  assert.match(explore, /renderLoadingPanels \? <ExploreLoadingPanel tab=\{tab\.id\} \/> : tabPanels\[tab\.id\]/);
  assert.match(explore, /<Reanimated\.View/);
  assert.match(explore, /<FlatList/);
...
test("mobile explore cards navigate to existing native detail routes", () => {
  ...
  assert.match(explore, /renderLoadingPanels \? EMPTY_PEOPLE : buildPeople\(posts, viewerName, viewerDisplayName\)/);
  assert.match(explore, /\[posts, renderLoadingPanels, viewerDisplayName, viewerName\]/);
...
test("mobile explore detail screens use the settings-style slide-over animation", () => {
  ...
  assert.match(people, /stack: \{\s*gap: spacing\.md/);
  ...
  assert.match(apiConfig, /if \(expoHost\) \{\s*url\.hostname = expoHost/);
  assert.doesNotMatch(apiConfig, /expoHost && !\/\^\(localhost\|127/);
...
test("mobile root and memory surfaces use the active theme", () => {
  ...
  assert.match(layout, /<StatusBar backgroundColor=\{themeColors\.bg\} style=\{resolvedTheme === "light" \? "dark" : "light"\} \/>/);
