...
test("mobile restaurant and dish detail routes use filtered feed queries", () => {
  ...
  assert.match(services, /rowItemMatchesDish\(item, input\)/);
  assert.match(services, /rowItemString\(candidate\.canonicalDishId\) === canonicalDishId/);
  assert.match(services, /function rowItemNameMatchesExactly/);
  assert.match(services, /normalizedCandidate === normalizedDishName/);
  assert.doesNotMatch(services, /dishSearchMatches/);
  assert.match(services, /location\?: ExploreFeedInput\["location"\]/);
  assert.match(services, /scanPublicReviewRows\(viewerName, \{[\s\S]+location[\s\S]+\}\)/);
  assert.match(services, /restaurantAddress\?: string \| null/);
