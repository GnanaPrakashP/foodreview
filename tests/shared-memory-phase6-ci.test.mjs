...
test("phase 6 status remains phase-gated or records completed hardening", () => {
  assert.match(status, /Current phase: (Production Hardening Phase 4|Production Hardening Phase 5|Production Hardening Phase 6|Production Hardening Phase 7|Phase 6|Complete|Final production-readiness audit|Production DB deployment verification)/);
  assert.match(status, /Next required (?:phase|work): (Phase 4 manual staging\/release matrix|Phase 5 manual staging\/performance matrix, followed by separately authorized Phase 6 work|Phase 6 hosted\/physical-device staging matrix, then separately authorized Phase 7 work|execute the Phase 7 hosted staging gate and the outstanding Phase 1A–6 hosted\/physical-device gates\. Phase 8 is not started by this handoff\.|Tests and CI\/CD|None|Production DB deployment verification|Authenticated staging smoke verification)/);
});
