...
test("mobile post and avatar uploads use the authorized upload/finalize flow", () => {
  ...
  assert.match(mobileReviewMedia, /fileSizeBytes: prepared\.body\.byteLength/);
...
test("profile statistics are server-derived and profile posts use stable keyset pagination", () => {
  ...
  assert.match(profileScreen, /<GestureHandlerScrollView/);
  assert.match(profileScreen, /\{profileHeader\}[\s\S]*profilePagerWindowStyle/);
  assert.match(profileScreen, /postRows\.map\(\(item\) =>/);
  assert.match(profileScreen, /memoriesRows\.map\(\(item\) =>/);
  assert.match(profileScreen, /updatePagerPageHeight\("posts", event\.nativeEvent\.layout\.height\)/);
  assert.match(profileScreen, /updatePagerPageHeight\("memories", event\.nativeEvent\.layout\.height\)/);
  assert.match(profileScreen, /onScroll=\{handleProfileScroll\}/);
  assert.match(profileScreen, /PROFILE_PAGINATION_EDGE_DISTANCE/);
  assert.match(profileScreen, /RefreshControl/);
  assert.match(profileScreen, /const profileHeaderTouchHandlers = useMemo\(\(\) => \(\{/);
  assert.match(profileScreen, /const profileHeaderSwipeHandlers = useMemo\(\(\) => PanResponder\.create\(\{/);
  assert.match(profileScreen, /openCreate\("profile-header-swipe"\)/);
  assert.match(profileScreen, /const profilePagerGesture = useMemo\(\(\) => Gesture\.Pan\(\)/);
  assert.doesNotMatch(profileScreen, /<Animated\.FlatList/);
  assert.doesNotMatch(profileScreen, /data=\{activeRows\}/);
  assert.doesNotMatch(profileScreen, /ListHeaderComponent=\{renderListHeader\}/);
  assert.doesNotMatch(profileScreen, /ListHeaderComponent=\{profileHeader\}/);
  assert.doesNotMatch(profileScreen, /<ProfilePager/);
  assert.doesNotMatch(profileScreen, /function ProfilePager/);
  assert.doesNotMatch(profileScreen, /pagingEnabled/);
