...
export async function signMemoryPhotoPayload(payload: JsonRecord, roomId: string) {
  ...
  const { data: assetRows, error: assetError } = assetIds.length > 0
    ? await admin
      .from("media_assets")
      .select("id, owner_id, owner_name, surface, media_type, status, access_class, visibility, privacy_state, moderation_status, consumed_at")
      .in("id", assetIds)
  ...
  const [{ data: legacySigned, error: legacySignError }, { data: privateSigned, error: privateSignError }] = await Promise.all([
  ...
  return {
    ...
    photos: photos.map((photo) => {
      ...
      return {
        ...
        mime_type: canonical?.mime_type ?? photo.mime_type ?? null,
        poster_url: posterUrl,
        public_url: canonicalUrl,
        signed_url_expires_at: canonicalUrl ? privateExpiresAt : null,
